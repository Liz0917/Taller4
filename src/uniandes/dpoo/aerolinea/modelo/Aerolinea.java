...
public class Aerolinea
{
    ...
    {
    	 IPersistenciaTiquetes persistencia;

    	    if (tipoArchivo.equals(CentralPersistencia.JSON)) {
    	        persistencia = new PersistenciaTiquetesJson();
    	    } else {
    	    ...
    	    try {
    	        ...
    	        throw new InformacionInconsistenteException("Información inconsistente en el archivo: " + e.getMessage());
    	    }
    }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (vuelo == null) {
            throw new Exception("No se encontró el vuelo correspondiente a la fecha y código de ruta especificados");
        }

        int capacidadDisponible = vuelo.getAvion().getCapacidad() - vuelo.getTiquetes().size();
        if (cantidad > capacidadDisponible) {
            throw new VueloSobrevendidoException("No hay suficientes asientos disponibles en el vuelo para la cantidad de tiquetes solicitados");
        }
        ...
        for (int i = 0; i < cantidad; i++) {
            int tarifa = 0;
			try {
				tarifa = CalculadoraTarifas.calcularTarifa(vuelo, cliente);
			} catch (Exception e) {
        ...
        // Retornar el valor total de los tiquetes vendidos
        return valorTotal;
    }
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        if (cliente != null) {
            Collection<Tiquete> tiquetesSinUsar = cliente.getTiquetesSinUsar();
            for (Tiquete tiquete : tiquetesSinUsar) {
                saldoPendiente += tiquete.getTarifa();
            }
        }

        return "El saldo pendiente es "+ saldoPendiente;
    }
}
